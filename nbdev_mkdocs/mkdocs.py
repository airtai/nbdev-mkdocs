...
# %% ../nbs/Mkdocs.ipynb 86
def _fix_sym_links(s: str, lib_name: str) -> str:
    """Fix the default sym links generated by nbdev in the given string.

    Args:
        s: The string to fix
        lib_name: Libary name for which the symlinks needs to be fixed.
    ...
    """
    pattern = f"\[`{lib_name}\.(?:\w+\.)*\w+`\]\(https?://\S+#.+\)"
    regex = re.compile(pattern)
    matches = regex.findall(s)
    for match in matches:
        fixed_part = "/".join(match.split("/")[:3])
        fixed_link = f"{fixed_part}/{lib_name}_api_docs/{lib_name}/{fixed_part.split('.')[1]}/#{fixed_part.split('`')[1]})"
        s = s.replace(match, fixed_link)
    return s

# %% ../nbs/Mkdocs.ipynb 90
def _fix_sym_links_in_nbs(root_path: str, cache_path: Path, lib_name: str) -> None:
    ...
    for file in files:
        ...
        for cell in _f.cells:
            if cell.cell_type == "markdown":
                updated_src = _fix_sym_links(cell["source"], lib_name)
...
def nbdev_mkdocs_docs(root_path: str, refresh_quarto_settings: bool = False) -> None:
    ...
    with set_cwd(root_path):
        ...
        lib_package_name = lib_name.replace("-", "_")

        cache_path = proc_nbs()
        _fix_sym_links_in_nbs(root_path, cache_path, lib_package_name)
