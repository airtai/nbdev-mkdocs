...
def _fix_sym_links(s: str, lib_name: str) -> str:
    ...
    pattern = f"\[`{package_name}\.(?:\w+\.)*\w+`\]\(https?://\S+#.+\)"
    regex = re.compile(pattern)
    matches = regex.findall(s)
    for match in matches:
        fixed_part = "/".join(match.split("/")[:-1])
        fixed_link = f"{fixed_part}/{package_name}_api_docs/{package_name}/{fixed_part.split('.')[1]}/#{fixed_part.split('`')[1]})"
...
def nbdev_mkdocs_docs(root_path: str, refresh_quarto_settings: bool = False) -> None:
    ...
    with set_cwd(root_path):
        ...
        cache_path = proc_nbs()
        _fix_sym_links_in_nbs(root_path, cache_path, lib_name)
