...
def _import_all_members(module_name: str) -> List[str]:
    ...
    members: List[Tuple[str, Union[types.FunctionType, Type[Any]]]] = sum(
        [_import_functions_and_classes(m) for m in submodules], start=[]
    )
...
def _add_all_submodules(members: List[str]) -> List[str]:
    ...
    submodules = list(set(sum([_f(x) for x in members], start=[])))
...
def _fix_sym_links(s: str, nbdev_lookup: NbdevLookup) -> str:  # type: ignore
    ...
    for match in re.findall(pattern, s):
        ...
        if symbol_details is not None:
            ...
            updated_link = f"{fixed_part}/{package_name}_api_docs/{package_name}/{symbol_details[1].split('/')[1].replace('.py', '')}/#{sym_path})"
            s = s.replace(match, updated_link)
    return s

# %% ../nbs/Mkdocs.ipynb 116
def _fix_sym_links_in_nbs(root_path: str, cache_path: Path, nbdev_lookup: NbdevLookup) -> None:  # type: ignore
    ...
    for file in files:
        ...
        for cell in _f.cells:
            if cell.cell_type == "markdown":
                updated_src = _fix_sym_links(cell["source"], nbdev_lookup)
                cell["source"] = updated_src

        nbformat.write(_f, file)

# %% ../nbs/Mkdocs.ipynb 118
def nbdev_mkdocs_docs(root_path: str, refresh_quarto_settings: bool = False) -> None:
    ...
    with set_cwd(root_path):
        ...
        cache_path = proc_nbs()
        nbdev_lookup = NbdevLookup(incl_libs=lib_name)
        _fix_sym_links_in_nbs(root_path, cache_path, nbdev_lookup)
