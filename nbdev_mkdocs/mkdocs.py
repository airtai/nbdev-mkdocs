...
# %% auto 0
__all__ = ['new', 'new_cli', 'replace_all', 'get_submodules', 'generate_api_doc_for_submodule', 'generate_api_docs_for_module',
...
# %% ../nbs/Mkdocs.ipynb 36
def replace_all(text, img_path):
    for p in img_path:
        if p in text:
            text = text.replace(p, "../../images/nbs")
    return text

# %% ../nbs/Mkdocs.ipynb 38
def _copy_guide_images_and_update_path(root_path: str):
    """Copy guide images to the docs directory and update its relative path in the markdown files
    ...
    cache = proc_nbs()
    nbs_images_path = [p for p in Path(cache).glob(r"**/*") if p.suffix in image_extensions]
    
    if len(nbs_images_path) > 0:
        doc_path = Path(root_path) / "mkdocs" / "docs"
        dst_img_path = Path(doc_path) / "images" / "nbs"
        dst_img_path.mkdir(exist_ok=True, parents=True)
        
        for src_img_path in nbs_images_path:
            shutil.copy(src_img_path, dst_img_path)

        nbs_images_path = [str(p.parent)[len(str(cache))+1:] for p in nbs_images_path]
        for nb in _get_docs_nbs(cache):
            dir_prefix = str(nb.parent)[len(str(cache))+1:]
            md = doc_path / f"{dir_prefix}" / f"{nb.stem}.md"

            with open(Path(md), "r") as f:
                _new_text = f.read()
                _new_text = replace_all(_new_text, nbs_images_path)
