...
# %% auto 0
__all__ = ['ALLOWED_DOCS_VERSIONING_VALUES', 'new', 'nbdev_mkdocs_docs', 'prepare', 'preview']
...
# %% ../nbs/Mkdocs.ipynb 86
def _get_docs_deploy_params(
    root_path: str, current_docs_version: str, docs_versioning: str
) -> Dict[str, str]:
    mkdocs_path = Path(root_path) / "mkdocs" / "mkdocs.yml"
    with _read_yaml_file(mkdocs_path) as (_, config):
        default_doc_tag = config["extra"]["version"]["default"]

    ret_val = {}
    ret_val["default_doc_tag"] = default_doc_tag

    if current_docs_version.replace(".", "").isdigit():
        ret_val["doc_deployment_version"] = (
            ".".join(current_docs_version.split(".")[:-1])
            if docs_versioning == "minor"
            else current_docs_version
        )
        ret_val["doc_deployment_tag"] = default_doc_tag
...
# %% ../nbs/Mkdocs.ipynb 89
ALLOWED_DOCS_VERSIONING_VALUES = ["None", "minor", "patch"]


def _update_deploy_yaml_if_docs_versioning_enabled(root_path: str) -> None:
    deploy_yaml_path = Path(root_path) / ".github" / "workflows" / "deploy.yaml"
    settings_path = Path(root_path) / "settings.ini"

    for file_path in [deploy_yaml_path, settings_path]:
        if not file_path.exists():
            typer.secho(
                f"Unexpected error: path {file_path.resolve()} does not exists!",
                err=True,
                fg=typer.colors.RED,
            )
            raise typer.Exit(code=1)
    ...
    if docs_versioning not in ALLOWED_DOCS_VERSIONING_VALUES:
        typer.secho(
            f"Error: Invalid value set for 'docs_versioning' in settings.ini file: '{docs_versioning}'. Allowed values are {ALLOWED_DOCS_VERSIONING_VALUES}",
            err=True,
            fg=typer.colors.RED,
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        typer.secho(
            f"Unexpected error while parsing deploy.yaml: {e}",
            err=True,
            fg=typer.colors.RED,
        )
        raise typer.Exit(code=1)
...
def nbdev_mkdocs_docs(root_path: str, refresh_quarto_settings: bool = False) -> None:
    ...
    with set_cwd(root_path):
        ...
        _update_deploy_yaml_if_docs_versioning_enabled(root_path)
