...
# %% auto 0
__all__ = ['set_cwd', 'new', 'new_cli', 'test_nb', 'custom_nbdev_test', 'get_submodules', 'generate_api_doc_for_submodule',
           'generate_api_docs_for_module', 'generate_cli_doc_for_submodule', 'generate_cli_docs_for_module',
           'build_summary', 'copy_cname_if_needed', 'prepare', 'prepare_cli', 'preview', 'preview_cli']
...
def test_nb(
    ...
    if str2bool(fm.get("skip_exec", False)) or nb_lang(nb) != "python":
        return True, 0

    def _no_eval(cell):
        if cell.cell_type != "code":
            return True
        if "nbdev_export" + "(" in cell.source:
            return True
        direc = getattr(cell, "directives_", {}) or {}
        if direc.get("eval:", [""])[0].lower() == "false":
            return True
    ...
    try:
        ...
        if showerr:
            sys.stderr.write(k.prettytb(fname=fn) + "\n")
        res = False
    if do_print:
...
def _keep_file(  # type: ignore
    ...
    if p.exists():
        return not bool(
            p.parent.ls().attrgot("name").filter(lambda x: x == ignore_fname)  # type: ignore
        )
    else:
        True


# %% ../nbs/api/test.ipynb 10
@call_parse
@delegates(nbglob_cli)  # type: ignore
def custom_nbdev_test(
    path: str = None,  # type: ignore
    flags: str = "",  # Space separated list of test flags to run that are normally ignored
    n_workers: int = None,  # type: ignore
    ...
    **kwargs,
    ...
    skip_flags = nbdev.config.get_config().tst_flags.split()  # type: ignore
    force_flags = flags.split()
    files = nbglob(path, as_path=True, **kwargs)  # type: ignore
    files = [f.absolute() for f in sorted(files) if _keep_file(f, ignore_fname)]
    if len(files) == 0:
        return print("No files were eligible for testing")
    if n_workers is None:
        n_workers = 0 if len(files) == 1 else min(num_cpus(), 8)  # type: ignore
    if IN_NOTEBOOK:  # type: ignore
        kw = {"method": "spawn"} if os.name == "nt" else {"method": "forkserver"}  # type: ignore
    else:
        kw = {}
    with working_directory(nbdev.config.get_config().nbs_path):  # type: ignore
        results = parallel(  # type: ignore
            ...
            n_workers=n_workers,
            basepath=nbdev.config.get_config().config_path,
            pause=pause,
            do_print=do_print,
            **kw,
        )
